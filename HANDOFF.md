...
- A later full Windows acceptance attempt was started after syncing this diagnostics change, but the SSH connection to the VM timed out before the final `acceptance_summary.json` could be verified. Do not treat that interrupted run as acceptance evidence.
- If the VM connection is unstable, use `tools\start_reachops_acceptance_background_windows.ps1` to launch acceptance in the background and `tools\get_reachops_acceptance_background_status_windows.ps1 -Json` to recover the PID, log tails, and `acceptance_summary.json` verification after reconnecting.
