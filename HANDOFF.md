...
- PM goal-mode status is authoritative through `tools\reachops_goal_delivery_runner.py --json`. Its `delivery_boundary` is the first field to read: `local_mvp_scope_ready=true` means Mac local MVP can be accepted, while `overall_final_delivery_scope_ready=false` means the whole project is not final-delivered.
...
- Historical Windows client delivery validation exists for the non-live path, but it is not current final-delivery evidence for this worktree. The current `/Users/aofa/Documents/New project` checkout has no `dist\ReachOps\ReachOps.exe`, no `dist\installer\ReachOps-Setup-0.4.0.exe`, no `dist\installer\reachops-update-manifest.json`, and no local `reports\reachops_acceptance\*\acceptance_summary.json`.
- The current local final package check returns `status=failed`, `final_delivery_ready=false`, with `exe_missing`, `installer_missing`, `manifest_missing`, and `acceptance_summary_missing`. Treat this as authoritative until the Windows VM regenerates the artifacts into the current repo.
...
- Final delivery must also include `windows_package_preflight.json`, `final_acceptance_gate.json`, and a strict `delivery_package_check.json` that validates both files. `--allow-missing-final-gate` is only for the Windows acceptance script bootstrap pass before `final_acceptance_gate.json` exists; that JSON is `bootstrap_only=true` and `final_delivery_ready=false`, so it is not a final delivery standard.
- Final delivery requires `tools\reachops_final_acceptance_gate.py --json` to return `status=passed`, `final_delivery_ready=true`, and `failed_checks=[]`.
