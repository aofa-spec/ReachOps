...
- Tests and checks:
  ...
  - PR #12 latest closeout audit: scope review reconfirmed `CampaignRun`, `Observation`, run traceability, evidence traceability, legacy migration compatibility, and idempotency are complete for this P1 runtime data-contract slice.
  - PR #12 latest closeout audit: `LeadDecision` remains intentionally limited to immutable `lead_decision_observations`; broader lead lifecycle semantics should stay split into a separate P3/P1 follow-up PR.
  - PR #12 latest closeout audit: legacy handling strategy remains explicit; pre-P1 batches get deterministic `legacy_run_<batch_id>` compatibility rows with `legacy_backfill=true` and `legacy_handling_strategy=deterministic_legacy_run_id_for_existing_batch_only`, while historical candidates, leads, actions, outreach executions, evidence paths, and errors keep blank `run_id`.
  - PR #12 latest closeout audit: `/usr/bin/python3 -m py_compile ReachOps/intelligence/storage.py ReachOps/intelligence/schemas.py ReachOps/intelligence/candidate_user_scorer.py ReachOps/workbench/workflow_service.py tests/test_campaign_run_observations.py tests/test_reachops_campaign.py tools/reachops_delivery_audit.py`: passed.
  - PR #12 latest closeout audit: `/usr/bin/python3 -m unittest -v tests.test_campaign_run_observations`: passed, 17 tests.
  - PR #12 latest closeout audit: `/usr/bin/python3 -m unittest -v tests.test_truthful_execution_semantics`: passed, 7 tests; log `/tmp/reachops-pr12-truth-latest.log`.
  - PR #12 latest closeout audit: `/usr/bin/python3 tools/reachops_operator_pressure.py --json`: passed, `status=ok`, `submitted_unverified=0`; output `/tmp/reachops-pr12-operator-pressure-latest.json`.
  - PR #12 latest closeout audit: `/usr/bin/python3 -m unittest -v tests.test_reachops_campaign`: failed with existing baseline shape, 230 tests, 14 failures and 1 error; branch log `/tmp/reachops-pr12-campaign-latest.log`.
  - PR #12 latest closeout baseline comparison: `origin/main` also ran 230 tests with 14 failures and 1 error; main log `/tmp/reachops-main-pr12-campaign-latest.log`; artifact `/tmp/reachops-pr12-baseline-comparison-latest.json`, `new_failures=[]`, `new_errors=[]`.
  - PR #12 latest closeout audit: `/usr/bin/python3 tools/reachops_delivery_audit.py --json`: failed, exit `1`, `status=failed`, summary `passed=47`, `pending_external_validation=3`, `failed=4`; failed checks are non-P1 P4/Web runtime delivery checks on this branch; output `/tmp/reachops-pr12-delivery-audit-latest.json`.
  - PR #12 latest closeout audit: `/usr/bin/python3 tools/reachops_goal_status_report.py --json`: failed, exit `1`, `status=failed`, summary `final_passed=28`, `final_pending_external_validation=3`, `final_failed=2`; output `/tmp/reachops-pr12-goal-status-latest.json`.
  - PR #12 latest closeout audit: `/usr/bin/python3 tools/reachops_delivery_package_check.py --json`: failed, exit `1`, `final_delivery_ready=false`, missing `exe`, `installer`, `manifest`, and `acceptance_summary`; output `/tmp/reachops-pr12-package-check-latest.json`.
  - PR #12 latest closeout audit: `/usr/bin/python3 tools/reachops_goal_delivery_runner.py --json`: failed, exit `1`, `status=not_ready`, `final_delivery_ready=false`; blocking scopes are `local_mvp`, `windows_final_artifacts`, and `external_authorized_execution`; output `/tmp/reachops-pr12-goal-delivery-runner-latest.json`.
  - PR #12 latest closeout audit: `/usr/bin/python3 tools/reachops_final_acceptance_gate.py --json`: failed, exit `1`, `status=failed`, `final_delivery_ready=false`; failed checks are `goal_status:passed`, `client_delivery:final_ready`, `delivery_package:passed`, and `delivery_audit:no_failed_checks`; output `/tmp/reachops-pr12-final-gate-latest.json`.
  - PR #12 latest closeout audit: `/usr/bin/python3 tools/reachops_repository_cleanliness_check.py --json`: passed, `forbidden_count=0`; output `/tmp/reachops-pr12-cleanliness-latest.json`.
  - PR #12 latest closeout audit: `git diff --check`: passed; log `/tmp/reachops-pr12-diff-check-latest.log`.
- Safety:
  - No real TikTok action was executed.
