...
## Latest P1 LeadDecision lineage export hardening

- Date: `2026-07-20`
- Branch: `codex/p4-web-runtime-smoke`
- Scope: Close the current P1 runtime data trust slice by making LeadDecision report/export lineage explicit without creating new architecture, entering Windows packaging, generating EXE/installer artifacts, or attempting TikTok live-submit.
- Code evidence:
  - `lead_decisions` now has an idempotent `decision_version` migration with default `1`.
  - `record_lead_decision(...)` preserves the existing no-overwrite/idempotency behavior for duplicate provider-version decisions and assigns the next version for the same `(campaign_id, run_id, candidate_observation_id, decision_key)` lineage.
  - `list_lead_decisions(...)` exposes run/campaign/observation-scoped lineage for reports.
  - Growth reports now include `lead_decisions`, `lead_decision_lineage_schema_version`, and `lead_decision_version_count`.
  - Report export now writes scoped LeadDecision lineage to JSON plus a separate `*_lead_decisions.csv`; Markdown traceability includes `decision_versions`.
  - Delivery audit now checks for the P1 LeadDecision lineage/export contract.
- Tests and checks:
  - `/usr/bin/python3 -m py_compile ReachOps/intelligence/schemas.py ReachOps/intelligence/storage.py ReachOps/intelligence/growth_reporter.py tools/reachops_delivery_audit.py tests/test_reachops_runtime_model.py`: passed; log `/tmp/reachops-lead-decision-lineage-pycompile.log`.
  - `/usr/bin/python3 -m unittest -v tests.test_reachops_runtime_model`: passed, 9 tests; log `/tmp/reachops-lead-decision-lineage-runtime-model.log`.
  - `/usr/bin/python3 -m unittest -v tests.test_truthful_execution_semantics`: passed, 7 tests; log `/tmp/reachops-lead-decision-lineage-truth.log`.
  - `/usr/bin/python3 -m unittest -v tests.test_reachops_campaign`: passed, 233 tests; log `/tmp/reachops-lead-decision-lineage-campaign.log`.
  ...
  - Branch/main campaign comparison: `/tmp/reachops-lead-decision-lineage-baseline-comparison.json` reports `new_failures=[]`, `new_errors=[]`.
  - `/usr/bin/python3 tools/reachops_operator_pressure.py --json`: passed, `status=ok`; output `/tmp/reachops-lead-decision-lineage-operator-pressure.json`.
  - `/usr/bin/python3 tools/reachops_delivery_audit.py --json`: passed, `status=ok`, summary `passed=51,pending_external_validation=3,failed=0`; output `/tmp/reachops-lead-decision-lineage-delivery-audit.json`.
  - `/usr/bin/python3 tools/reachops_goal_status_report.py --json`: passed as `ready_for_external_validation`, summary `final_passed=30,final_pending_external_validation=3,final_failed=0`; output `/tmp/reachops-lead-decision-lineage-goal-status.json`.
  - `/usr/bin/python3 tools/reachops_client_delivery_check.py --json`: failed as expected, `status=not_started`, `readiness=not_started`, failed check `acceptance:ready`; output `/tmp/reachops-lead-decision-lineage-client-delivery.json`.
  - `/usr/bin/python3 tools/reachops_delivery_package_check.py --json`: failed as expected, `final_delivery_ready=false`, missing `exe`, `installer`, `manifest`, and `acceptance_summary`; output `/tmp/reachops-lead-decision-lineage-package-check.json`.
  - `/usr/bin/python3 tools/reachops_final_acceptance_gate.py --json`: failed as expected, `status=not_ready`, `final_delivery_ready=false`; failed checks are `goal_status:passed`, `client_delivery:final_ready`, and `delivery_package:passed`; output `/tmp/reachops-lead-decision-lineage-final-gate.json`.
  - `/usr/bin/python3 tools/reachops_goal_delivery_runner.py --json`: failed as expected, `status=not_ready`, `final_delivery_ready=false`; blocking scopes are `local_mvp`, `windows_final_artifacts`, and `external_authorized_execution`; output `/tmp/reachops-lead-decision-lineage-goal-delivery-runner.json`.
  - `/usr/bin/python3 tools/reachops_repository_cleanliness_check.py --json`: passed, `forbidden_count=0`; output `/tmp/reachops-lead-decision-lineage-cleanliness.json`.
- Safety:
