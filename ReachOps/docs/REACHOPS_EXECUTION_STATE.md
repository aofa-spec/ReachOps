...
- Code evidence:
  ...
  - `list_observations_for_run(...)` now returns observation rows plus run-scoped queued actions, outreach executions, events, and errors so action proposals, evidence paths, and error context are traceable by run from one audit payload.
