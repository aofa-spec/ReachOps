...
最终验收包合同：Windows acceptance 必须写出 `windows_package_preflight.json`、`authorization_handoff_payload.json`、`latest_reachops_authorization_handoff.zip`、`issue_closure_payload.json`、`final_acceptance_gate.json`，并能追溯 `reports\acceptance_remediation\latest_delivery_check.json`；`acceptance_summary.json` 必须写入 `windows_package_preflight`、`authorization_handoff`、`client_delivery`、`issue_closure` 和 `final_acceptance_gate`。`delivery_package_check.py` 和 `final_acceptance_gate.py` 会把这些报告作为最终 `report_files` 必备证据。最终门禁还必须通过 `commercial_issue_closure:closed`。
