...
class GrowthOpsConsole(ttk.Frame):
    ...
    def _build_start_collection_page(self, frame):
        ...
        ttk.Label(rule_box, text="每个目标最多视频").grid(row=1, column=0, sticky="w", pady=(6, 0))
        ttk.Spinbox(rule_box, from_=1, to=20, width=5, textvariable=self.scan_max_videos_var).grid(row=1, column=1, sticky="w", pady=(6, 0))
        ttk.Label(rule_box, text="每条视频最多评论").grid(row=1, column=2, sticky="w", pady=(6, 0))
        ttk.Spinbox(rule_box, from_=1, to=50, width=5, textvariable=self.scan_max_comments_var).grid(row=1, column=3, sticky="w", pady=(6, 0))
        ttk.Label(rule_box, text="参与账号数").grid(row=1, column=4, sticky="w", pady=(6, 0))
        ttk.Spinbox(rule_box, from_=1, to=50, width=5, textvariable=self.scan_profile_limit_var).grid(row=1, column=5, sticky="w", pady=(6, 0))
        ...
        ttk.Label(rule_box, text="排除词").grid(row=2, column=2, sticky="w", pady=(6, 0))
        ttk.Entry(rule_box, textvariable=self.scan_exclude_keywords_var).grid(row=2, column=3, columnspan=3, sticky="ew", pady=(6, 0))
        ttk.Label(rule_box, text="评论回复").grid(row=3, column=2, sticky="w", pady=(6, 0))
        ttk.Combobox(
            rule_box,
            ...
            state="readonly",
            width=16,
        ).grid(row=3, column=3, columnspan=3, sticky="ew", pady=(6, 0))
