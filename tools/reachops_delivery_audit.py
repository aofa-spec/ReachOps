...
def run_web_local_api_architecture_fixture() -> dict:
    ...
    checks = {
        ...
        "p1_runtime_observation_ledger_contract": "CREATE TABLE IF NOT EXISTS campaign_runs" in growth_storage and "CREATE TABLE IF NOT EXISTS candidate_observations" in growth_storage and "CREATE TABLE IF NOT EXISTS evidence_artifacts" in growth_storage and "CREATE TABLE IF NOT EXISTS lead_decisions" in growth_storage and "legacy rows must not be assigned a fabricated run_id" in growth_storage and "runtime_traceability_summary" in growth_storage and "legacy_run_id_fabricated\": False" in growth_storage and "record_candidate_observation" in growth_storage and "record_evidence_artifact" in growth_storage and "record_lead_decision" in growth_storage and "create_campaign_run" in growth_task_router and "bind_collection_batch_run" in growth_task_router and "active_run_id" in growth_task_router and "build_report(campaign_id=campaign_id, run_id=active_run_id, batch_id=batch_id)" in growth_task_router and "record_candidate_observation" in operation_lead_manager and "record_evidence_artifact" in operation_lead_manager and "record_lead_decision" in operation_lead_manager and "reachops.runtime_traceability_report.v1" in growth_reporter and "_resolve_runtime_scope" in growth_reporter and "runtime_traceability" in growth_reporter and "legacy_rows_preserved" in growth_reporter and "\"run_id\"" in growth_reporter and "\"campaign_id\"" in growth_reporter and "start_contract_current_evidence_complete" in (ROOT_DIR / "tools" / "reachops_goal_delivery_runner.py").read_text(encoding="utf-8"),
