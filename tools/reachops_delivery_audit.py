...
def run_web_local_api_architecture_fixture() -> dict:
    ...
    checks = {
        ...
        "headless_updates_run_session_checkpoints": "parser.add_argument(\"--run-session\"" in headless and "update_run_session(\"PRECHECK\"" in headless and "state=\"COLLECTING\"" in headless and "update_runtime_checkpoint" in headless and "runtime_state_inferred" in headless and "log_line_count" in headless and "result[\"status\"] == \"completed\"" in headless and "\"COMPLETED\"" in headless and "\"BLOCKED\"" in headless and "execution_plan_contract" in run_session and "merged_evidence[\"execution_plan_contract\"]" in run_session,
        ...
        "native_app_entry_unifies_to_web_with_explicit_legacy_tk_diagnostics": "from ReachOps.launcher import main" in app_entry and "_launch_web_client" in (ROOT_DIR / "ReachOps" / "launcher.py").read_text(encoding="utf-8") and "--web" in (ROOT_DIR / "ReachOps" / "launcher.py").read_text(encoding="utf-8") and "REACHOPS_WEB_CLIENT" in (ROOT_DIR / "ReachOps" / "launcher.py").read_text(encoding="utf-8") and "--help" in (ROOT_DIR / "ReachOps" / "launcher.py").read_text(encoding="utf-8") and "Show this help without starting clients" in (ROOT_DIR / "ReachOps" / "launcher.py").read_text(encoding="utf-8") and "未知启动参数" in (ROOT_DIR / "ReachOps" / "launcher.py").read_text(encoding="utf-8") and "Start the native Tk client" in (ROOT_DIR / "ReachOps" / "launcher.py").read_text(encoding="utf-8") and "return 2" in (ROOT_DIR / "ReachOps" / "launcher.py").read_text(encoding="utf-8") and "回退到原生 Tk 客户端" not in (ROOT_DIR / "ReachOps" / "launcher.py").read_text(encoding="utf-8"),
        ...
        "client_entry_copy_identifies_local_client_console": "启动ReachOps本地客户端.command" in (ROOT_DIR / "README.md").read_text(encoding="utf-8") and "启动ReachOps统一WebUI.command" in (ROOT_DIR / "README.md").read_text(encoding="utf-8") and "启动ReachOps本地客户端.command" in (ROOT_DIR / "ReachOps" / "README.md").read_text(encoding="utf-8") and "启动ReachOps统一WebUI.command" in (ROOT_DIR / "ReachOps" / "README.md").read_text(encoding="utf-8") and (ROOT_DIR / "启动ReachOps本地客户端.command").exists() and "ReachOpsApp.py" in (ROOT_DIR / "启动ReachOps本地客户端.command").read_text(encoding="utf-8") and "原生客户端 UI" in (ROOT_DIR / "启动ReachOps本地客户端.command").read_text(encoding="utf-8") and "启动ReachOps本地客户端.command" in (ROOT_DIR / "tools" / "sync_reachops_to_windows_vm.sh").read_text(encoding="utf-8") and "该入口不会打开 Web 控制台" in (ROOT_DIR / "启动ReachOps原生MacUI.command").read_text(encoding="utf-8") and "ReachOps 本地客户端控制台实际地址" in (ROOT_DIR / "启动ReachOps统一WebUI.command").read_text(encoding="utf-8") and "ReachOps 本地客户端控制台" in (ROOT_DIR / "ReachOps" / "launcher.py").read_text(encoding="utf-8") and "Web 运营面板" not in mac_self_check,
